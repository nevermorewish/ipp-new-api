...
func OriginalModelPrice2JSONString() string {
	return originalModelPriceMap.MarshalJSONString()
}

func UpdateOriginalModelPriceByJSONString(jsonStr string) error {
	return types.LoadFromJsonStringWithCallback(originalModelPriceMap, jsonStr, InvalidateExposedDataCache)
}

// GetOriginalModelPrice returns optional direct USD-per-million list prices
// used only for display. These values never participate in billing.
func GetOriginalModelPrice(name string) (OriginalModelPrice, bool) {
	name = FormatMatchingModelName(name)
	price, ok := originalModelPriceMap.Get(name)
	return price, ok
}
...
func GetOriginalModelPriceCopy() map[string]OriginalModelPrice {
	return originalModelPriceMap.ReadAll()
}
