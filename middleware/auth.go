...
func authHelper(c *gin.Context, minRole int) {
	...
	if !useAccessToken {
		...
		currentUser, refreshErr := model.GetUserById(userId, false)
		if refreshErr != nil {
			if errors.Is(refreshErr, gorm.ErrRecordNotFound) {
				...
				common.SysLog("failed to refresh session user during authentication")
				c.JSON(http.StatusInternalServerError, gin.H{
					"success": false,
					"message": common.TranslateMessage(c, i18n.MsgDatabaseError),
				})
...
func TokenAuth() func(c *gin.Context) {
	return func(c *gin.Context) {
		...
		if len(allowIps) > 0 {
			...
			if ip == nil {
				abortWithOpenAiMessage(c, http.StatusForbidden, "无法解析终端 IP 地址")
				return
			}
